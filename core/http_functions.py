...
async def get_upload_url(cloud_path: str) -> Optional[str]:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url=url, headers=headers, params=params) as response:
            ...
            if status_code != 200:
                logger.error(f"ошибка загрузки файла в облако {file_name} статус код {status_code}")
                return
...
async def save_file_to_cloud(file_path: str):
    ...
    while count < 5:
        ...
        logger.error("кол-во попыток на получение URL для загрузки файла в облако превышено")
        return
...
async def delete_file_in_cloud(file_path: str):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.delete(url=url, headers=headers, params=params) as response:
            ...
            if status_code != 204:
                logger.error(f"файл {file_name} не был удален из облака, статус код {status_code}")
                return
    return True
